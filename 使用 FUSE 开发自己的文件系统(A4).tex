...
\setCJKmonofont[Scale=0.9,Path=Fonts/]{SourceCodePro-Light}
\setmainfont[Path=Fonts/]{SourceHanSansCN-Normal}
\setsansfont[Path=Fonts/]{SourceHanSansCN-Normal}
\setmonofont[Scale=0.9,Path=Fonts/]{SourceCodePro-Light}
...
\item
  ...
  \item
    ...
    若函数出错, 原样返回错误值;
  \item 
      获取进程对该文件的读写权限, 并与打开标志 \texttt{flag} 作比较,
      如果互相冲突 (例如进程对文件只可读, 但打开标志包含写), 则返回 
      \verb'-EACCES';
  \item
    若打开标志 \texttt{flag} 包含写操作 (\texttt{O\_WRONLY},
    \texttt{O\_RDWR} 等), 而 \texttt{path} 是一个目录文件, 返回
    \texttt{-EISDIR};
  \item
    若 \texttt{flag} 指定了 \texttt{O\_DIR}, 但 \texttt{path}
    不是一个目录文件, 返回 \texttt{-ENOTDIR};
  \item
    若 \texttt{flag} 指定了 \texttt{O\_TRUNC}, 则调用
    \texttt{ufs\_truncatei(inode)} 与 \texttt{ufs\_wr\_inode(inode)},
    若函数出错, 原样返回错误值;
  \item
